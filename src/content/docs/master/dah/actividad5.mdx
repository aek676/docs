...
Esta actividad consiste en implementar pruebas en una aplicación Ionic existente. La aplicación de ejemplo es un sistema de gestión de salón de belleza que permite gestionar clientes, reservas y productos. Se implementan pruebas unitarias utilizando Jasmine con Karma, pruebas de componentes con Cypress y pruebas E2E (end-to-end) con Cypress.
...
Para esta sesión, utilizaremos Jasmine con Karma para las pruebas unitarias, y Cypress para las pruebas unitarias, pruebas de integración entre componentes, y de interfaz de usuario (E2E).
...
Para la ejecución de las pruebas unitarias, vamos a hacer uso de Cypress y también revisaremos cómo poder llevar a cabo este tipo de pruebas con Jasmine. Anteriormente, las pruebas implementadas con Jasmine se ejecutaban con Karma, aunque ahora se recomienda migrar a soluciones más modernas como Web Test Runner o Vitest.
...
Para la ejecución de las pruebas unitarias, Jasmine se utiliza para la implementación de los tests, mientras que Karma, Web Test Runner o Vitest se utilizan para la ejecución de dichos tests.
...
<RemoteCode
  url={`${REPO_URL}src/app/core/services/generate-text.service.ts`}
  lang="typescript"
...
<GitDiff
  ...
  commitA="aee438b9"
  commitB="60f11105"
  file="actividad5/src/app/core/services/generate-text.service.spec.ts"
  lang="typescript"
/>
...
Enquanto que se realiza la prueba de este componente, el acceso al mismo se realiza a través de un manejador (fixture) que permite hacer referencia a una instancia de dicho componente (`fixture.componentInstance`). Para acceder a los elementos del DOM, se hará uso de `fixture.nativeElement`.

<GitDiff
  repo="aek676/dah-2026"
  commitA="5f0a377d"
  commitB="60f11105"
  file="actividad5/src/app/features/home/home.page.ts"
  lang="typescript"
/>
...
<GitDiff
  ...
  commitA="5f0a377d"
  commitB="5f0a377d"
  file="actividad5/src/app/features/home/home.page.spec.ts"
  lang="typescript"
/>
...
Chrome 146.0.0.0 (Windows 10) is idle
Jasmine 4.6.1
9 specs, 0 failures, randomized with seed 45285
GenerateTextService
  should do nothing
  should have "DAH' and 'service' in the list of words
  should return a random word as a string
  should be created
  should return a non-empty array
AppComponent
  should create the app
HomePage
  should update the word in the DOM when button clicked.
  initialize the title from signal
  should update the title in the DOM when changeTitle is called
Finished in 0.1295
```

Si alguna de las pruebas falla, se nos mostrará información sobre la causa que ha provoca el fallo.
...
Si ocurre un error debido a la ruta de la aplicación, se debe modificar el archivo `cypress.config.ts` para configurar a mano la ruta:

<RemoteCode
  url={`${REPO_URL}cypress.config.ts`}
  lang="typescript"
  title="cypress.config.ts"
/>

<RemoteCode
  url={`${REPO_URL}cypress/support/component.ts`}
  lang="typescript"
  title="cypress/support/component.ts"
/>
...
La configuración E2E ya incluye `baseUrl: 'http://localhost:8100'` como se mostró anteriormente.

</Aside>

## 5. Tests implementados

A continuación se enumerate los tests implementados en la aplicación:

### 5.1. Tests con Jasmine (Karma)

- Tests del servicio GenerateTextService
- Tests del componente HomePage
...
- Acceso al detalle de cliente y显示 del ID
- Botón atrás desde detalle de cliente
- Acceso a crear reserva
- Botón de cerrar sesión que redirige a login
